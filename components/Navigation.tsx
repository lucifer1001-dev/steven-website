...
export default function Navigation() {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${isScrolled || isOpen ? 'border-b border-white/10 bg-[#071126]/75 shadow-[0_10px_40px_rgba(2,6,23,.24)] backdrop-blur-2xl' : 'bg-transparent'}`}>
      <nav className="section-shell flex h-20 items-center justify-between" aria-label="Primary navigation">
        <Link href="/" className="group flex items-center gap-3" aria-label="Steven Chen, home">
          <span className="border-r border-violet-300/40 pr-3 font-mono text-sm font-semibold tracking-[0.08em] text-violet-300">SC</span>
